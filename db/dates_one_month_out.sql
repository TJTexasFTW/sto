...
WHERE  extract(month from blocked_date) = extract(month from CURRENT_DATE) + 1
  AND extract(year from blocked_date) = extract(year from (CURRENT_DATE + INTERVAL '1 month'))
...
WHERE  extract(month from event_date) = extract(month from CURRENT_DATE) + 1
  AND extract(year from event_date) = extract(year from (CURRENT_DATE + INTERVAL '1 month'))
